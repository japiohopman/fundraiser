...
export function openContextModal(state, contextData, lang, triggerEl, item) {
  ...
  let provenanceHTML = '';
  if (contextData.provenanceBadge || contextData.provenanceNote) {
    provenanceHTML += '<div class="context-modal-provenance-wrapper">';
    if (contextData.provenanceBadge) {
      const badgeText = contextData.provenanceBadge[lang] || contextData.provenanceBadge.nl;
      provenanceHTML += `<span class="provenance-badge">${badgeText}</span>`;
    }
    if (contextData.provenanceNote) {
      ...
      provenanceHTML += `<p class="provenance-note">${formattedNote}</p>`;
    }
    provenanceHTML += '</div>';
  }
  ...
  if (contextData.text) {
    ...
    textHTML += `<p class="context-modal-text">${formattedMainText}</p>`;
  }
  ...
  if (contextData.paragraphs && Array.isArray(contextData.paragraphs)) {
    contextData.paragraphs.forEach(pObj => {
      ...
      textHTML += `<p class="context-modal-text">${formattedPText}</p>`;
    });
  }
  ...
  if (donateUrl || hasLink) {
    actionsHTML += `<div class="context-modal-actions">`;
  ...
  if ((item?.id === 'rooie-jaap-knives' || (item?.id === undefined && equipmentRef)) && equipmentRef && state.contentData?.rooieJaapEquipment) {
    equipmentHTML = createRooieJaapEquipmentHTML(state.contentData.rooieJaapEquipment, lang, equipmentRef);
  }

  const hasMedia = Boolean(headerImageHTML || galleryHTML);
  const gridClass = hasMedia ? 'context-modal-grid has-media' : 'context-modal-grid no-media';
  ...
  html += `<div class="context-modal-primary-col">`;
  html += textHTML;
  html += equipmentHTML;
  html += actionsHTML;
  html += `</div>`;
  ...
  if (hasMedia) {
    html += `<div class="context-modal-media-col">`;
