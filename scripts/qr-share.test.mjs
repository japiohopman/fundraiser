...
// Ensure global document/window fallbacks exist for top-level bindings and element rendering
...
if (typeof globalThis.document === 'undefined') {
  ...
  if (!globalThis.document.createElement) {
    globalThis.document.createElement = createDummyElement;
  }
}
...
test('Context Modal renders generic Donate button using donateUrl with localized labels', () => {
  const bodyEl = createDummyElement();
  const modalEl = createDummyElement();
  const titleEl = createDummyElement();
...
test('Unified QR Modal opens with correct content, thank-you message above QR, and actions below QR for campaign and site sharing', () => {
  const elements = {
    'qr-modal': createDummyElement(),
    'qr-modal-title': createDummyElement(),
    'qr-modal-campaign-name': createDummyElement(),
    'qr-modal-thank-you': createDummyElement(),
    'qr-modal-desc': createDummyElement(),
    'qr-code-svg-container': createDummyElement(),
    'qr-modal-url-text': createDummyElement(),
    'qr-modal-close-btn': createDummyElement(),
    'qr-modal-native-share-btn': createDummyElement(),
    'site-share-btn': { id: 'site-share-btn' }
  };

  const originalDoc = globalThis.document;
  globalThis.document = {
    getElementById: (id) => elements[id] || createDummyElement(),
    createElement: createDummyElement,
    addEventListener: () => {},
    removeEventListener: () => {}
  };
...
test('Language switch closes open QR modal before re-rendering in new language', async () => {
  const { setLanguage } = await import('../app.js');
  ...
  const elements = {
    ...
    'qr-modal-native-share-btn': createDummyElement(),
    'site-share-btn': { id: 'site-share-btn' },
    'unified-fundraisers-list': createDummyElement(),
    'timeline-list': createDummyElement(),
    'sources-list': createDummyElement(),
    'donor-wall': createDummyElement(),
    'campaign-stats-strip': createDummyElement()
  };
  ...
  try {
    ...
    // Open QR modal in NL
    openQRModal(state, 'Test Title NL', 'https://example.com', null);
    assert.equal(state.isQRModalOpen, true);
    ...
    // Reopen modal and verify EN content is loaded
    openQRModal(state, 'Test Title EN', 'https://example.com', null);
    assert.equal(elements['qr-modal-thank-you'].textContent, contentData.thankYou.message.en);
  } finally {
    globalThis.document = originalDoc;
